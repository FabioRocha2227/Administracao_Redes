...
R: O ```Term1``` tenta resolver endereço de ```dns.admredes.pt``` (como é um servidor caching aceita pedidos recursivos e vai resolver-los iterativamente).Logo segue os passos: falar com servidor raiz para descobrir servidores com **autoridade** sobre **dominio** ```.pt```, depois o contacta um desser servidores com ```autoridade sobre .pt``` para saber se conhecem algum servidor com **autoridade** sobre **dominio** ```admredes.pt``` (como não conhecem nenhum servidor que tenha autoridade sobre esse dominio, aparece erro ```Host admredes.pt not found: 3(NXDOMAIN)```).Importante  reparar que admredes.pt é um dominio privado (e não esta registado em nenhum servido com autoridade sobre o dominio ```.pt```)
...
R: O ``registo cola`` permite resolver a **dependencia ciclica** que ocorre quando delegamos ```subdominios``` dentro de um dado ```dominio``` (basicamente consiste num ```registo A``` que é associado ```delegação de subdominio feita```).Foi necessário utilizar nesta montagem no ```dns.admredes.pt``` que delegou o **subdominio** ```dept.admredes.pt``` ao servidor ```dns.dept.admredes.pt```.

![alt text](image-19.png)

>**Exemplo (SEM REGISTO COLA)**

![alt text](image-19.jpeg)
...
+ Criar ficheiro ```admredes.pt-all.zone```

    + cd /var/named/master/

    + touch admredes.pt-all.zone

        + chgrp named admredes.pt-all.zone

        + chmod 640 admredes.pt-all.zone

>**duvida** não sei quais configurações seriam necessarias para que 172.16.0.0/24 sejam vistos como estando na rede 192.168.80.0/24
...
**TODO: corrigir erro que da alinea a e fazer configurações que faltam**
...
+ ```Resposta``` do ```MASTER``` para o ```SLAVE``` com a respetiva **transferência de zona**
...
**duvida**: como é que copio o ficheiro asksalve.pcap do GNS3 para a VM???
